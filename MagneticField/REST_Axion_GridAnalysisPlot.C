...
struct FieldTrack {
    std::unique_ptr<TRestAxionMagneticField> magneticField; // Using unique_ptr
    std::unique_ptr<TRestAxionField> axionField; // Using unique_ptr
...
Int_t REST_Axion_GridAnalysis(Int_t nData = 3, Double_t Ea = 4.2, std::string gasName = "He", Double_t m1 = 0.01, Double_t m2 = 0.1) {
    // Mesh Map Definitions in mm
    ...
    for(const auto& fieldName : fieldNames) {
        for (const auto& ma : masses) {
            ...
            for(unsigned i = 0; i < nData; i++) {
                if(kDebug){
                    std::cout << "+--------------------------------------------------------------------------+" << std::endl;
                    std::cout << "Data: " << i << "/t Mass: " << ma << std::endl;
                    std::cout << "+--------------------------------------------------------------------------+" << std::endl;
                    std::cout << std::endl;
                }
                for(auto& field : fields) {
